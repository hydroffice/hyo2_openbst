...
def run(raw_input, calib_input):
    ...
    # 7 because of selecting some of the fields
    data_bathy = np.empty((nr_pings, nr_rx_beams, 7))
    data_bathy[:] = np.nan
    ...
    # 3 because we read just 3 of the fields
    position_measured = np.empty((nr_dg_position, 3))
