...
class x7kRead:
    ...
    def read7k(self, verbose=True):
        ...
        if self.split_record:
            self.infile.seek(36,
                             1)  # this takes into account the netframe header for the next read, but doesn't fix the problem
            self.hypack_hdr -= 36
            self.split_record = False
...
class Data7001:
    ...
    def read_data(self):
        ...
        for i in range(self.header[1]):
            device_data = list(
                struct.unpack(self.fmt_data, self.datablock[self.datapointer:self.datapointer + self.data_sz]))
...
class Data7028:
    ...
    def read_data(self):
        ...
        if self.header[4] == 0:
            ...
            self.snippets = np.zeros((int(self.maxbeam), int(self.maxwindow)))
            for beam in range(self.numpoints):
...
class mappack:
    ...
    def printmap(self):
        ...
        for key in keys:
            ...
            print(
                'message ' + str(key[0]) + ' has ' + str(key[1]) + ' packets and ' + str(0.01 * percent) + '% of file')
